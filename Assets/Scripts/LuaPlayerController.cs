...
public class LuaPlayerController : MonoBehaviour
{
    public TextAsset luaScript;  // 可以在Inspector中指定，或者通过代码加载
    [Tooltip("如果不指定luaScript，将使用此路径从Lua文件夹加载")]
    public string luaScriptPath = "Game/Player/PlayerController.lua";
    ...
    void Awake()
    {
        ...
        if (!LoadLuaScript())
        {
            Debug.LogError("无法加载Lua脚本，LuaPlayerController无法工作");
            return;
        }
    ...
    private bool LoadLuaScript()
    {
        ...
        try 
        {
            ...
            if (luaBytes != null && luaBytes.Length > 0)
            {
                ...
                actualLoadedPath = luaScriptPath;
                return true;
            }
        }
    ...
    private void InitLuaEnvironment()
    {
        ...
        {            
            ...
            // 执行Lua脚本
            object[] results = sharedLuaEnv.DoString(luaScript.text, "LuaPlayerScript", scriptEnv);
            ...
            if (results != null && results.Length > 0 && results[0] is LuaTable)
            {
                ...
                luaUpdate = module.Get<LuaFunction>("Update");
                ...
                {
                    Debug.LogError("无法获取Lua函数: Start=" + (luaStart != null));
                }
            }
            else
            {
    ...
    void Update()
    {
        if (initialized && luaUpdate != null)
        {
            ...
            {
                Debug.LogError("调用Lua Update函数时出错: " + e.Message);
